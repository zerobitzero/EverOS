...
class EpisodicMemoryEsRepository(BaseRepository[EpisodicMemoryDoc]):
    ...
    async def multi_search(
        ...
        """
        BM25 text search for episodic memories
        ...
        try:
            ...
            if query:
                ...
                # Build should clauses - search across multiple text fields
                should_queries = []
            ...
            logger.debug("episodic memory search query: %s", search.to_dict())
            ...
            logger.debug(
                "Episodic memory search succeeded: query=%s, user_id=%s, found %d results",
                query,
                user_id,
                len(hits),
            )
            ...
            logger.error(
                "Episodic memory search failed: query=%s, user_id=%s, error=%s",
                query,
                user_id,
                e,
            )
    ...
    async def delete_by_filters(
        ...
        try:
            ...
            logger.info(
                "Batch deleted episodic memories: user_id=%s, group_id=%s, deleted %d records",
                user_id,
                group_id,
                deleted_count,
            )
            ...
            logger.error(
                "Failed to batch delete episodic memories: user_id=%s, group_id=%s, error=%s",
                user_id,
                group_id,
                e,
            )
